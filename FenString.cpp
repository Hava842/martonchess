...
Position FenString::toPosition(const std::string& fen) {
  Position position;
...
std::string FenString::fromPosition(const Position& position) {
  std::string fen;
  ...
  // Castling
  std::string castlingFenString;
  if ((position.castlingRights & Castling::WHITE_KINGSIDE) != Castling::NOCASTLING) {
    castlingFenString += fromCastling(Castling::WHITE_KINGSIDE);
  }
  if ((position.castlingRights & Castling::WHITE_QUEENSIDE) != Castling::NOCASTLING) {
    castlingFenString += fromCastling(Castling::WHITE_QUEENSIDE);
  }
  if ((position.castlingRights & Castling::BLACK_KINGSIDE) != Castling::NOCASTLING) {
    castlingFenString += fromCastling(Castling::BLACK_KINGSIDE);
  }
  if ((position.castlingRights & Castling::BLACK_QUEENSIDE) != Castling::NOCASTLING) {
    castlingFenString += fromCastling(Castling::BLACK_QUEENSIDE);
  }
  if (castlingFenString.empty()) {
    fen += '-';
  } else {
    fen += castlingFenString;
  }
...
int FenString::toColor(char notation) {
  char lowercaseFenString = std::tolower(notation);
  switch (lowercaseFenString) {
    case WHITE_NOTATION:
      return Color::WHITE;
    case BLACK_NOTATION:
      return Color::BLACK;
...
char FenString::fromColor(int color) {
  switch (color) {
    case Color::WHITE:
...
int FenString::colorOf(char notation) {
...
char FenString::transform(char notation, int color) {
  switch (color) {
    case Color::WHITE:
...
int FenString::toPieceType(char notation) {
  char uppercaseFenString = std::toupper(notation);
  switch (uppercaseFenString) {
    case PAWN_NOTATION:
...
char FenString::fromPieceType(int piecetype) {
  switch (piecetype) {
    case PieceType::PAWN:
...
int FenString::toPiece(char notation) {
...
char FenString::fromCastlingType(int castlingtype) {
  switch (castlingtype) {
...
int FenString::toCastling(char notation) {
  int color = colorOf(notation);
...
char FenString::fromFile(int file) {
  switch (file) {
    case File::a:
      return a_NOTATION;
    case File::b:
...
int FenString::toRank(char notation) {
  switch (notation) {
    case r1_NOTATION:
      return Rank::r1;
    case r2_NOTATION:
...
char FenString::fromRank(int rank) {
  switch (rank) {
    case Rank::r1:
      return r1_NOTATION;
    case Rank::r2:
...
int FenString::toSquare(const std::string& notation) {
...
std::string FenString::fromSquare(int square) {
  assert(Square::isValid(square));
